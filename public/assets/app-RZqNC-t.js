...
/**
 * RESET FLAGS COMPLET (IMPORTANT FIX SLIDERS)
 */
function resetInitFlags(root) {
    ...
    // IMPORTANT : sécurité sliders double range
    root.querySelectorAll(".double-slider").forEach(el => {
        el.dataset.sliderInitialized = "0";
    });
}
...
/**
 * SLIDERS DOUBLE RANGE (FIX IMPORTANT)
 */
function initSliders(root = document) {
    root.querySelectorAll(".double-slider").forEach(slider => {
        // toujours safe après AJAX
...
/**
 * BADGES (IMPORTANT FIX : toujours recréé)
 */
...
EventBus.on("ui:updated", () => {
    console.log("[UI] updated event received");
});
...
function boot() {
    initVehicleUsageTypeConfirmation(document);
}
