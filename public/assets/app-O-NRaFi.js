...
function initFetchForms(root = document) {
    root.querySelectorAll("[data-module='fetch-form']").forEach(form => {
        ...
        // Stocker l'instance pour la pagination globale
        form.__fetchFormInstance = new FetchForm(form);
    });
}
...
function initPagination() {
    document.addEventListener("click", e => {
        ...
        if (!form || !form.__fetchFormInstance) return;

        form.__fetchFormInstance.goToPage(link.dataset.page);
    });
}
