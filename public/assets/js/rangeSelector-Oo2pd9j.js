...
export default function initDoubleSlider(slider) {
  ...
  // Crée la structure HTML du slider si elle n’existe pas
  ...
  // Conversion valeur -> position en pixels
  const valueToPos = value =>
    ((value - MIN) / (MAX - MIN)) * slider.clientWidth;

  // Conversion position -> valeur arrondie à STEP
  ...
  // Met à jour visuel + inputs + labels + dispatch event
  const updateUI = () => {
    ...
    // Met à jour labels si target définie
    const targetMin = slider.dataset.targetMin
      ? document.querySelector(slider.dataset.targetMin)
      : null;
    const targetMax = slider.dataset.targetMax
      ? document.querySelector(slider.dataset.targetMax)
      : null;
    if (targetMin) targetMin.textContent = currentMin.toLocaleString("fr-FR");
    if (targetMax) targetMax.textContent = currentMax.toLocaleString("fr-FR");

    // Met à jour inputs cachés
  ...
  // Expose méthode reset accessible depuis l'extérieur
  slider.resetSlider = () => {
    currentMin = MIN;
    currentMax = MAX;
    updateUI();
  ...
  // Ajuste le slider si la fenêtre change
