// --- vehiclesFilters.js (adaptation toggle view) ---
...
export default class VehiclesFilter {
  ...
  initEvents() {
    ...
    // Changement sur le toggle view
    const viewSwitch = document.querySelector('[data-target="view-switch"]');
    if (viewSwitch) {
      viewSwitch.addEventListener("change", e => {
        if (e.target.name === "view") {
          // Ajoute la valeur dans le formulaire comme input caché
          let input = this.form.querySelector('input[name="filters[view]"]');
          if (!input) {
          ...
          // Sauvegarde session côté serveur via AJAX et recharge résultats
          this.submitFilters();
        }
      });
    }
