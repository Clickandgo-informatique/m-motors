...
export default class FilterBadges {
  ...
  /**
   * Initialise le listener pour supprimer un filtre
   */
  init() {
  ...
  /**
   * Génère les badges dynamiquement depuis l'objet filters
   * @param {Object} filters
   */
  render(filters) {
    this.container.innerHTML = ""; // vide container
    ...
    entries.forEach(([key, value]) => {
      if (Array.isArray(value)) {
        value.forEach(val => this.createBadge(key, val));
      } else {
        this.createBadge(key, value);
      }
    });
  }

  /**
   * Crée un badge et l'injecte dans le container
   */
  createBadge(filter, value) {
    const badge = document.createElement("span");
    badge.className = "badge badge-remove";
