export default class Dropzone {
  constructor(element) {
    // =========================================================
    // GLOBAL INSTANCE GUARD (ANTI DOUBLE INIT ABSOLU)
    // =========================================================
    if (window.__DROPZONE_INSTANCES__ === undefined) {
      window.__DROPZONE_INSTANCES__ = new WeakSet();
    }

    if (window.__DROPZONE_INSTANCES__.has(element)) {
      return;
    }

    window.__DROPZONE_INSTANCES__.add(element);
  ...
  // =========================================================
  // INIT (ANTI DOUBLE EVENT BINDING)
  // =========================================================
  init() {
    if (this.el.dataset.eventsBound) return;
    ...
    this.input.addEventListener("change", e => {
      ...
      // 🔥 IMPORTANT FIX DOUBLE INPUT TRIGGER
      this.input.value = "";
    });
  ...
  // =========================================================
  // ADD TO QUEUE (SAFE SINGLE EXECUTION)
  // =========================================================
  addToQueue(files) {
  ...
  // =========================================================
  // SERVER MERGE (NO DUPLICATE DOM)
  // =========================================================
  mergeUploaded(files) {
    files.forEach(file => {
      const selector = `.dz-thumb[data-id="${file.id}"]`;

      if (this.previewContainer.querySelector(selector)) {
        return;
      }
