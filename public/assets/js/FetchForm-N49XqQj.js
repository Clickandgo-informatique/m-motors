export default class FetchForm {
  ...
  renderPagination(data) {
    ...
    // compteur
    const counter = this.form.querySelector("[data-pagination-counter]");
    ...
    // boutons prev
    const prevButtons = this.form.querySelectorAll("[data-page-prev]");
    prevButtons.forEach(btn => {
      btn.disabled = current <= 1;
      btn.dataset.page = current - 1;
    });

    // boutons next
    const nextButtons = this.form.querySelectorAll("[data-page-next]");
    ...
    // boutons numérotés
    const pageButtons = this.form.querySelectorAll("[data-page]");
