export default class FilterBadges {
  constructor(container, form, onRemoveCallback) {
    ...
    if (!(this.container instanceof HTMLElement)) {
      console.warn("[FilterBadges] container invalide");
      return;
    }

    if (!(this.form instanceof HTMLFormElement)) {
      console.warn("[FilterBadges] form invalide ou introuvable");
      return;
    }
  ...
  resetSlider(filter) {
    ...
    const defaultMin = parseInt(sliderEl.dataset.min || "0", 10);
    const defaultMax = parseInt(sliderEl.dataset.max || "0", 10);
  ...
  updateBadges() {
    ...
    for (const [key, value] of formData.entries()) {
      ...
      this.createBadge(name, value, label || value);
    }
  }
