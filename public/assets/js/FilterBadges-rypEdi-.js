export default class FilterBadges {
    ...
    resetSlider(filter) {
        ...
        // 2. reset slider UI (IMPORTANT)
        sliderEl.dispatchEvent(
            new CustomEvent("sliderReset", {
                bubbles: true,
                detail: {
                    filter,
                    min: defaultMin,
        ...
        // 3. force update slider state si ton composant écoute change/input
    ...
    renderRangeBadges() {
        ...
        for (const [filter, [minKey, maxKey]] of Object.entries(rangeFilters)) {
            ...
            const isReset =
                ...
                min === "" ||
                max === "" ||
                (parseInt(min) === parseInt(defaultMin) && parseInt(max) === parseInt(defaultMax));

            const finalMin = isReset ? defaultMin : min;
            const finalMax = isReset ? defaultMax : max;

            const label = this.formatRangeLabel(filter, finalMin, finalMax);

            this.createBadge(filter, `${finalMin}-${finalMax}`, label, true);
        }
    }
