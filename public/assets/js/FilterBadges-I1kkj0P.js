export default class FilterBadges {
    ...
    for (const [filter, [minKey, maxKey]] of Object.entries(rangeFilters)) {
        ...
        const slider = this.form.querySelector(`.double-slider[data-filter="${filter}"]`);

        if (slider) {
            const defaultMin = slider.dataset.min;
            const defaultMax = slider.dataset.max;

            // Si min/max = valeurs par défaut → pas de badge
            if (String(min) === String(defaultMin) && String(max) === String(defaultMax)) {
                continue;
            }
        }
