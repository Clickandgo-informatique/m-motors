// vehiclesFilters.js

/**
 * Module de gestion des filtres véhicules + pagination AJAX
 * Compatible avec formulaire chargé dynamiquement (fragment sidebar)
 */
export default class VehiclesFilter {
  /**
   * @param {HTMLFormElement} form
   */
  constructor(form) {
    console.log("INIT VehiclesFilter → élément reçu :", form);

    // Vérification stricte : on doit recevoir un <form>
    if (!(form instanceof HTMLFormElement)) {
      console.error("VehiclesFilter : élément invalide (pas un form)", form);
      return;
    }

    this.form = form;

    // Vérification dataset
    if (!this.form.dataset) {
      console.error("VehiclesFilter : dataset introuvable");
      return;
    }

    // URL AJAX obligatoire
    this.url = this.form.dataset.fetchUrl;
    if (!this.url) {
      console.error("VehiclesFilter : data-fetch-url manquant");
      return;
    }

    console.log("URL AJAX :", this.url);

    // Initialisation des événements
    this.initEvents();
  }

  /**
   * Initialise tous les événements
   */
  initEvents() {
    console.log("Init events");

    /**
     * Écoute globale sur le formulaire
     * Gère tous les inputs (checkbox, hidden, slider, etc.)
     */
    this.form.addEventListener("change", e => {
      if (!e.target.matches("input")) return;

      console.log("Changement détecté :", e.target.name, e.target.value);

      this.submitFilters();
    });

    /**
     * Pagination via délégation d’événement
     */
    document.addEventListener("click", e => {
      const btn = e.target.closest("[data-page]");
      if (!btn) return;

      e.preventDefault();

      const page = parseInt(btn.dataset.page);
      if (isNaN(page)) return;

      console.log("Pagination demandée :", page);

      this.submitFilters(page);
    });
  }

  /**
   * Construit les filtres + envoie AJAX
   */
  async submitFilters(page = 1) {
    console.log("SubmitFilters → page :", page);

    const formData = new FormData(this.form);
    const filters = {};

    /**
     * Transformation des champs FormData vers un objet exploitable côté Symfony
     *
     * Exemple de transformation :
     *   filters[brand][]     → brand: []
     *   filters[bodyType][]  → bodyType: []
     *   filters[mileageMin]  → mileageMin: value
     */
    for (const [key, value] of formData.entries()) {
      // Extraction du nom réel du filtre
      const match = key.match(/^filters\[(.+)\](\[\])?$/);

      // Ignore tout ce qui ne correspond pas au format filters[]
      if (!match) continue;

      const cleanKey = match[1];

      // Cas tableau (checkbox)
      if (key.endsWith("[]")) {
        if (!filters[cleanKey]) {
          filters[cleanKey] = [];
        }

        filters[cleanKey].push(value);
      } else {
        // Cas valeur simple (input hidden, slider, etc.)
        filters[cleanKey] = value;
      }
    }

    console.log("Filters envoyés (corrigés) :", filters);

    /**
     * Appel AJAX vers Symfony
     */
    try {
      const response = await fetch(this.url, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
        body: JSON.stringify({
          filters: filters,
          page: page
        })
      });

      const data = await response.json();

      console.log("Réponse AJAX :", data);

      this.updateDOM(data);
    } catch (error) {
      console.error("Erreur AJAX :", error);
    }
  }

  /**
   * Mise à jour du DOM avec les fragments retournés
   */
  updateDOM(data) {
    console.log("Update DOM");

    // Résultats véhicules
    const results = document.querySelector("#vehicles-results");
    if (results && data.results) {
      results.innerHTML = data.results;
    }

    // Pagination top
    const top = document.querySelector(".pagination-wrapper.top");
    if (top && data.paginationTop) {
      top.innerHTML = data.paginationTop;
    }

    // Pagination bottom
    const bottom = document.querySelector(".pagination-wrapper.bottom");
    if (bottom && data.paginationBottom) {
      bottom.innerHTML = data.paginationBottom;
    }
  }
}

/**
 * Observer du DOM
 * Permet d'initialiser automatiquement le module lorsque le formulaire apparaît
 * (cas du chargement via fragment AJAX dans la sidebar)
 */
function watchFiltersForm() {
  console.log("Observer démarré");

  const observer = new MutationObserver(() => {
    const form = document.querySelector("#filters-form");

    // Formulaire non encore présent
    if (!form) return;

    // Vérification stricte
    if (!(form instanceof HTMLFormElement)) {
      console.error("Element trouvé mais pas un form :", form);
      return;
    }

    // Empêche double initialisation
    if (form.dataset.initialized) return;

    console.log("Form détecté → initialisation");

    form.dataset.initialized = "true";

    new VehiclesFilter(form);
  });

  observer.observe(document.body, {
    childList: true,
    subtree: true
  });
}

/**
 * Lancement automatique
 */
document.addEventListener("DOMContentLoaded", () => {
  watchFiltersForm();
});
