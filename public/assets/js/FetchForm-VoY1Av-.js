export default class FetchForm {
  ...
  async send() {
    ...
    try {
      ...
      /*
       * règle importante :
       * - les filtres doivent reset page
       * - mais la pagination doit rester fonctionnelle
       * => page est envoyée telle quelle sauf filtre actif
       */
...
document.addEventListener('click', function (e) {
    const btn = e.target.closest('[data-reset-filters]');
    if (!btn) return;

    const wrapper = btn.closest('[data-fetch-form]');
    if (!wrapper) return;

    const form = wrapper.querySelector('form');
    if (!form) return;

    // reset natif des champs
    form.reset();
    ...
    // déclenche une recherche AJAX après reset
