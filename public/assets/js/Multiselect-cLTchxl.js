...
export default class Multiselect {
  ...
  async fillAvalaibleItems() {
    ...
    this.availableItemsDiv.insertAdjacentHTML("beforeend", '<ul class="list-wrapper">');
    this.availableItems.forEach(item => {
      this.availableItemsDiv.insertAdjacentHTML(
        "beforeend",
        `<span data-id="${item.id}">${item.label}</span>`
      );
    });
    this.availableItemsDiv.insertAdjacentHTML("beforeend", "</ul>");
  }
