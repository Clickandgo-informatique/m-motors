export default function initDoubleSlider(slider) {
  ...
  // Récupération stricte des bornes (pas de fallback silencieux)
  const MIN = parseInt(slider.dataset.min, 10);
  const MAX = parseInt(slider.dataset.max, 10);
  const STEP = Number(slider.dataset.step) || 1;

  // Sécurité : si dataset invalide, on stoppe l'initialisation
  if (Number.isNaN(MIN) || Number.isNaN(MAX)) {
    console.error("[DoubleSlider] dataset.min/max invalide", slider.dataset);
    return;
  }
  ...
  // reset propre sans recréation DOM
  slider.resetSlider = () => {
    currentMin = MIN;
    currentMax = MAX;
    updateUI();
