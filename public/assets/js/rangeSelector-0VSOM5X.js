...
export default function initDoubleSlider(slider) {
  ...
  // Création HTML interne si nécessaire
  ...
  // Mise à jour visuelle et inputs du formulaire
  const updateUI = () => {
    ...
    // Inputs liés au formulaire parent
    const form = slider.closest("form");
    const externalMin = form.querySelector(
      `input[name="filters[${FILTER}Min]"]`
    );
    const externalMax = form.querySelector(
      `input[name="filters[${FILTER}Max]"]`
    );
    if (externalMin) externalMin.value = currentMin;
    if (externalMax) externalMax.value = currentMax;

    // Dispatch événement custom pour VehiclesFilter.js
