...
export default class VehiclesFilter {
   ...
   */
  updateDOM(data) {
    const container = document.getElementById("vehicles-results");
    if (!container) return;
    ...
    // Pagination Top
    const top = container.querySelector('[data-target="pagination-top"]');
    if (top && data.paginationTop) top.innerHTML = data.paginationTop;

    // Pagination Bottom
    const bottom = container.querySelector('[data-target="pagination-bottom"]');
    if (bottom && data.paginationBottom)
      bottom.innerHTML = data.paginationBottom;

    // Badges
    const summary = container.querySelector('[data-target="filters-summary"]');
    if (summary) {
      summary.innerHTML =
    ...
    // Re-initialiser le double-slider si présent
    const slider = this.form.querySelector(".double-slider");
