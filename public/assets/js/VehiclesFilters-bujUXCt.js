...
export default class VehiclesFilters {
  constructor(form, store) {
    ...
    // Vérification du store (sécurité minimale)
    if (!this.store || typeof this.store.update !== "function") {
      console.error("VehiclesFilters : store invalide ou manquant");
    }

    // Conteneurs DOM principaux
  ...
  /**
   * Bind des événements principaux du formulaire
   */
  bindEvents() {
    // Gestion des champs input/select
  ...
  /**
   * Synchronise le formulaire vers le store global
   */
  syncFormToState() {
  ...
  /**
   * Navigation vers fiche véhicule au clic sur une card
   */
  bindCardClick() {
