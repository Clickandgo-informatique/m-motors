export default function initDoubleSlider(slider) {
  ...
  if (!Number.isFinite(MIN) || !Number.isFinite(MAX) || MIN >= MAX) {
    console.error("[DoubleSlider] invalid dataset", slider.dataset);
    return;
  }
  ...
  const valueToPos = v => ((v - MIN) / (MAX - MIN)) * slider.clientWidth;
  ...
  thumbs.min.addEventListener("mousedown", e => {
    active = thumbs.min;
    document.onmousemove = e => drag(e, active);
    document.onmouseup = () => document.onmousemove = null;
  });

  thumbs.max.addEventListener("mousedown", e => {
    active = thumbs.max;
    document.onmousemove = e => drag(e, active);
    document.onmouseup = () => document.onmousemove = null;
  });
