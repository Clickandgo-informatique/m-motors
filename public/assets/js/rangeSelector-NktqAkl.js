export default function initDoubleSlider(slider) {
  ...
  function scheduleFetch() {
    ...
    slider._t = setTimeout(() => {
      ...
      // IMPORTANT: force reflow des inputs dans le DOM avant fetch
      inputMinEl?.setAttribute("value", currentMin);
      inputMaxEl?.setAttribute("value", currentMax);

      // déclenche input + change (double sécurité FetchForm)
      inputMinEl?.dispatchEvent(new Event("input", { bubbles: true }));
      inputMaxEl?.dispatchEvent(new Event("input", { bubbles: true }));
