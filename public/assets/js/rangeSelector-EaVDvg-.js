export default function initDoubleSlider(slider) {
    if (slider.dataset.sliderInitialized === "1") {
        // recalcul état visuel si déjà existant
        update();
        return;
    }
    ...
    /*
     * API PUBLIQUE — utilisée par reset bouton
     */
    ...
    slider.reset = function() {
        ...
        if (inputMinEl) inputMinEl.value = MIN;
        if (inputMaxEl) inputMaxEl.value = MAX;
    ...
    slider.set = function(min, max) {
        [currentMin, currentMax] = sanitize(min, max);
