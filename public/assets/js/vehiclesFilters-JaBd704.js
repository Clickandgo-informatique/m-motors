...
// Gestion du click sur les cards des véhicules
document.addEventListener("DOMContentLoaded", () => {
  ...
  itemsCards.forEach(item =>
    item.addEventListener("click", e => {
      // On récupère l'élément qui a l'attribut data-item-link
      const card = e.target.closest("[data-item-link]");
      if (!card) return;

      const link = card.dataset.itemLink; // <-- ici on lit data-item-link
      console.log("Card cliquée :", link);

      // Exemple : redirection vers le lien
