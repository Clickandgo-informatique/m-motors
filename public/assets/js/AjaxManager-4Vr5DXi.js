...
export default class AjaxManager {
  ...
  /**
   * Enregistrement des listeners globaux
   */
  bindEvents() {
    /**
     * Ouverture modale via clic sur élément avec data-ajax-modal
     */
    document.addEventListener("click", e => {
      ...
      // Nettoyage éventuel des dropdowns autocomplete
    ...
    document.addEventListener("submit", async e => {
      ...
      if (response.ok) {
        ...
        if (el) {
          ...
          console.warn("AjaxManager: élément cible introuvable", target);
        }
      }
    });
   ...
   * Priorité :
   * 1. data-item-link
   * 2. data-ajax-modal (peut contenir directement une URL)
   * 3. href
   */
  resolveUrl(element) {
    const url =
  ...
  /**
   * Ajout d'un élément de collection Symfony
   */
  ...
  /**
   * Suppression d'un élément de collection Symfony
   */
