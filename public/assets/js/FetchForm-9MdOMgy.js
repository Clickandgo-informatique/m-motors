...
export default class FetchForm {
  ...
  bindEvents() {
    ...
    this.input.addEventListener("keydown", e => {
      const items = this.resultDiv.querySelectorAll(".dropdown-item");
      ...
      if (e.key === "Enter") {
        e.preventDefault();
        const link = items[this.activeIndex]?.querySelector(".dropdown-link");
  ...
  renderResults(data, q) {
    ...
    if (!data.length) {
      this.resultDiv.innerHTML += `<div class="dropdown-no-results">Aucun résultat</div>`;
      this.resultDiv.classList.add("active");
      return;
    }
    ...
    data.forEach(item => {
      ...
      let text = Object.entries(item)
        .filter(([key, val]) => typeof val === "string" && key !== "url")
        .map(([_, val]) => val)
        .join(" ");
      ...
      div.innerHTML = `
                <a href="${item.url}" class="dropdown-link">
                    ${text}
                </a>
            `;
  ...
  updateActiveItem(items) {
    ...
    const activeLink = items[this.activeIndex]?.querySelector(".dropdown-link");
