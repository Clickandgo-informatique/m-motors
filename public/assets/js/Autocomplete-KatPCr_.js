export default class Autocomplete {
  ...
  select(item) {
    ...
    console.log("[Autocomplete] input value set:", this.input.value);

    this.clear();
    this.close();

    console.log("[Autocomplete] dispatch change event");
  ...
  syncHidden(value) {
    ...
    console.log("[Autocomplete] syncHidden value:", value);
    console.log("[Autocomplete] form found:", form);

    if (!form) return;

    const hidden =
      form.querySelector("[data-autocomplete-value]") ||
      form.querySelector('input[type="hidden"][name="q"]');

    console.log("[Autocomplete] hidden resolved:", hidden);

    if (!hidden) {
      console.warn("[Autocomplete] STILL NO HIDDEN FIELD");
      return;
    }

    hidden.value = value;

    console.log("[Autocomplete] hidden SET:", hidden.value);
  }
  clear() {
