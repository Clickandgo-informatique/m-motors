 ...
 *
 * Objectif :
 * - Initialiser tous les modules de manière isolée
 * - Empêcher qu’un module casse toute l’app (important)
 * - Garantir que FetchForm et events UI fonctionnent toujours
 * - Réinitialisation propre après AJAX (ui:updated)
 * ------------------------------------------------------------------
 */
...
function initApp() {
  ...
  const modules = [
    ["sidebar", () => initSidebar?.()],
    ["dropzones", initDropzones],
    ["filters", initFilters],
    ["favorites", initFavorites],
    ["autocomplete", initAutocomplete],
    ["fetchforms", initFetchForms],
    ["collections", initCollections],
    ["ajaxManager", initAjaxManager],
    ["sliders", initSliders]
  ];
  ...
  modules.forEach(([name, fn]) => {
    try {
      ...
      console.error(`[${name.toUpperCase()} ERROR]`, e);
    }
  });
