...
class VehicleModelFixtures extends Fixture
{
    ...
    public function load(ObjectManager $em): void
    {
        ...
        // 🔹 Format de la barre avec pourcentage
        ...
        while (($row = fgetcsv($file, 0, ';')) !== false) {
            ...
            $fuelName = $this->normalizeFuel($this->col($row, $map, 'energ'));
            ...
            if ($variant) $vm->setVariant($variant);
            if ($fuel) $vm->setFuelType($fuel);
            ...
            $vm->setCo2($this->sanitizeNumber($this->col($row, $map, 'co2_mixte'), 2000));

            // MASSES
            $massMin = $this->sanitizeNumber($this->col($row, $map, 'masse_ordma_min'), 10000);
            $massMax = $this->sanitizeNumber($this->col($row, $map, 'masse_ordma_max'), 10000);
            if ($massMin && $massMax && $massMin > $massMax) $massMin = null;
            ...
            // 🔹 LIAISONS CORRECTES (côté propriétaire uniquement)
            // FuelType est lié via $vm->setFuelType()
            // Brand est lié via $vm->setBrand()
            // BodyType est lié via $vm->setBodyType()
    ...
    {
        return empty($this->bodyTypes) ? null : $this->bodyTypes[array_rand($this->bodyTypes)];
    }
    ...
    private function getModel(ObjectManager $em, Brand $brand, string $name): Model
    {
        ...
        if (!$model) {
            $model = (new Model())->setName($name)->setBrand($brand);
            $em->persist($model);
        }
    ...
    private function getVariant(ObjectManager $em, Model $model, ?string $name): ?Variant
    {
        ...
        if (!$variant) {
            $variant = (new Variant())->setName($name)->setModel($model);
            $em->persist($variant);
        }
    ...
    private function getFuel(ObjectManager $em, string $name): FuelType
    {
        ...
        if (!$fuel) {
            $fuel = (new FuelType())->setName($name);
            $em->persist($fuel);
        }
